        ...
        <div class="row bodyheight">
          ...
          <div class="col-10 border-start border-secondary">
            <c:if test="${not (sessionScope.user eq 'activ')}">
              <c:if test="${requestScope.body eq 'singin'}">
                <c:import url="/WEB-INF/jsp/singin.jsp" />
              </c:if>
              <c:if test="${requestScope.body eq 'registration'}">
                <c:import url="/WEB-INF/jsp/registration.jsp" />
              </c:if>
              <!-- <c:if test="${requestScope.body eq 'result_reg'}">
                <c:import url="/WEB-INF/jsp/result_registration.jsp" />
              </c:if> -->
            </c:if>
          </div>
        </div>

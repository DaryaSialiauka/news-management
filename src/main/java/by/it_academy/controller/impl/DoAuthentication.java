...
public class DoAuthentication implements Command {
	...
	private final static Logger LOG = LogManager.getLogger(DoAuthentication.class);
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(idUser == 0) {
				...
				userNotAuthenticatedRedirect(request, response);
			}
		...
		} catch (ValidationAuthenticationException | FindUserServiceException e) {
			LOG.error(e);
			userNotAuthenticatedRedirect(request, response);
	...
	private static void userNotAuthenticatedRedirect(HttpServletRequest request, HttpServletResponse response) throws IOException {
		HttpSession session = request.getSession(true);
		...
		url.append(JSPPageName.SING_IN_PAGE).append(AttributeAndParameter.SEPARATOR).append(AttributeAndParameter.BODY)
				...
				.append(AttributeAndParameter.EQUALS).append(AttributeAndParameter.TRUE).append(AttributeAndParameter.SEPARATOR)

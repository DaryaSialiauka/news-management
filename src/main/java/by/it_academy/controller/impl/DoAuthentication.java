...
public class DoAuthentication implements Command {
	...
	private static void userNotAuthenticatedRedirect(HttpServletRequest request, HttpServletResponse response, String message) throws IOException {
		HttpSession session = request.getSession(true);
		...
		String path = JSPPageName.SING_IN_ERROR_PAGE + AttributeAndParameter.SEPARATOR + AttributeAndParameter.BODY + AttributeAndParameter.EQUALS
				...
				+ message + AttributeAndParameter.SEPARATOR + AttributeAndParameter.ERROR_CLASS_STYLE + AttributeAndParameter.EQUALS
